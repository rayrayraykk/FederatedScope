        ...
        for ((b = 0; b < ${#batch_sizes[@]}; b++)); do
          for ((y = 0; y < ${#layers[@]}; y++)); do
            for ((h = 0; h < ${#hiddens[@]}; h++)); do
              for k in {1..3}; do
                FILE="out_fedopt/mlp/${out_dir}_${sample_rates[$sr]}/lr${lrs[$l]}_wd${wds[$w]}_dropout0_step${steps[$s]}_batch${batch_sizes[$b]}_layer${layers[$y]}_hidden${hiddens[$h]}_lrserver${lrs_server[$sl]}_momentumsserver${momentums_server[$ms]}_seed${k}"
                if [ -d "$FILE" ]; then
                  echo "$FILE exists."
                else

...
def main():
    ...
    with gr.Blocks() as demo:
        ...
        out_block = None
        # gr.Textbox(label='Output', lines=10, interactive=False)

        with gr.Accordion("Instrument panel", open=True):
            with gr.Tab("Room List"):
                room_df.render()
                lobby_disp_input = gr.Textbox(
                    label='Display Condition (optional)',
                    placeholder='dict/{}')
            ...
            with gr.Tab("Discover"):
                ...
                with gr.Box():
                    ...
                    with gr.Row():
                        lobby_disc_input_column = gr.Number(
                            label='Target Column', value=1)
                        lobby_disc_input_extract = gr.Dropdown(
                            ['DP', 'SS', 'HE'],
                            value=['DP'],
                            label='Encryption')
        ...
        with gr.Accordion("Console", open=True):
            with gr.Tab("Lobby"):
                with gr.Accordion("Authorization", open=IS_OPEN_ACCORDION):
                    with gr.Row():
                        lobby_auth_input_idx = gr.Number(label='Room ID',
                                                         value=1)
                        lobby_auth_input_password = gr.Textbox(
                            label='Password', type='password')
                    lobby_auth_btn = gr.Button("Get Authorization")
                    lobby_auth_btn.click(room_manager.authorize,
                                         inputs=[
                                             lobby_auth_input_idx,
                                             lobby_auth_input_password
                                         ],
                                         outputs=out_block)
                    with gr.Row():
                        lobby_auth_app_idx = gr.Number(label='Applicant ID',
                                                       value=1)
                        lobby_auth_app_action = gr.Dropdown(
                            ['agree', 'reject'],
                            value=['agree'],
                            label='Action')
                ...
                with gr.Accordion("Create Room", open=IS_OPEN_ACCORDION):
                    with gr.Accordion("Basic settings", open=True):
                        with gr.Row():
                            ...
                            with gr.Column():
                                lobby_add_scenario = gr.Dropdown(
                                    ['vFL', 'hFL'],
                                    value=['vFL'],
                                    label='Scenario')
                    ...
                    add_basic_input = [
                        ...
                        lobby_add_scenario, lobby_add_domain,
                        lobby_add_input_yaml, lobby_add_input_private,
                        lobby_add_input_opts, lobby_add_input_password
                    ]
                    ...
                    with gr.Accordion("Tuning Center", open=IS_OPEN_ACCORDION):
                        ...
                        with gr.Box():
                            ...
                            with gr.Row():
                                lobby_add_tune_min_lr = gr.Slider(
                                    0, 1, value=0.0, label='Minimum')
                                lobby_add_tune_max_lr = gr.Slider(
                                    0, 1, value=0.0, label='Maximum')
                        with gr.Box():
                            gr.Markdown("Weight Decay")
                            with gr.Row():
                                lobby_add_tune_min_wd = gr.Slider(
                                    0, 1, value=0.0, label='Minimum')
                                lobby_add_tune_max_wd = gr.Slider(
                                    0, 1, value=0.0, label='Maximum')
            ...
            with gr.Tab("ECS"):
                with gr.Accordion("Join Room", open=True):
                    with gr.Row():
                        ecs_add_input_idx = gr.Number(label='Room ID',
                                                      value='1')
                        ecs_add_input_ip = gr.Textbox(label='IP Address',
                                                      placeholder='127.0.0.1')
                        ecs_add_input_opts = gr.Textbox(

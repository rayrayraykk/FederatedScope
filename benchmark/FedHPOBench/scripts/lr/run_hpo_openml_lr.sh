        ...
        do
            ...
            do
                ...
                do
                    for k in {1..3}
                    do
                        python federatedscope/main.py --cfg benchmark/FedHPOBench/scripts/lr/openml_lr.yaml device $cudaid data.splitter_args \[\{\'alpha\'\:${alpha}\}\] train.optimizer.lr ${lrs[$l]} train.optimizer.weight_decay ${wds[$w]} train.local_update_steps ${steps[$s]} data.type ${dataset}@openml data.batch_size ${batch_sizes[$b]} federate.sample_client_rate ${sample_rates[$sr]} model.out_channels $out_channels seed $k outdir lr_alpha${alpha}/${out_dir}_${sample_rates[$sr]} expname lr${lrs[$l]}_wd${wds[$w]}_dropout0_step${steps[$s]}_batch${batch_sizes[$b]}_seed${k} >/dev/null 2>&1
                    done
                done
            done
        done
